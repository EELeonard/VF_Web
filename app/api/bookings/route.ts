...
export async function POST(request: Request) {
  try {
    ...
    if(dayAssignment){const toMinutes=(value:string)=>{const [hours,minutes]=value.split(":").map(Number);return hours*60+minutes;};const withinRange=toMinutes(flightTime)>=toMinutes(dayAssignment.available_from)&&toMinutes(flightTime)+duration<=toMinutes(dayAssignment.available_until)+1;if(withinRange){const conflict=await db.prepare("SELECT reference FROM bookings WHERE instructor_id=? AND id!=? AND status!='cancelled' AND flight_start_at<? AND datetime(flight_start_at,'+'||duration||' minutes')>? LIMIT 1").bind(dayAssignment.instructor_id,insert.meta.last_row_id,candidateEndAt,flightStartAt).first();if(!conflict)await db.prepare("UPDATE bookings SET instructor_id=?,instructor_assignment_source='day' WHERE id=?").bind(dayAssignment.instructor_id,insert.meta.last_row_id).run();}}
