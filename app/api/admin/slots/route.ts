...
import { ensureAvailabilityDatabase, isBookingTime, isSimulator, type AppointmentSlot } from "../../../lib/availability-db";
...
export async function PUT(request: Request) {
  if (!(await isValidSession(request, env.DB))) return Response.json({ error: "Nicht autorisiert." }, { status: 401 });
  const body = await request.json() as { simulator?: string; date?: string; time?: string; enabled?: boolean };
  const simulator = String(body.simulator ?? "");
  const flightDate = String(body.date ?? "");
  const flightTime = String(body.time ?? "");
  if (!isSimulator(simulator) || !/^\d{4}-\d{2}-\d{2}$/.test(flightDate) || !isBookingTime(flightTime) || typeof body.enabled !== "boolean") {
