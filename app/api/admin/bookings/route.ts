...
export async function POST(request: Request) {
  ...
  const flightStartAt = viennaLocalToUtc(flightDate, flightTime), candidateWindow = bookingOperationalWindow(flightStartAt, duration);
  ...
  const db = await ensureInstructorDatabase(env.DB);
  if (await db.prepare("SELECT id FROM bookings WHERE simulator=? AND status!='cancelled' AND datetime(flight_start_at, '-' || CASE WHEN duration=30 THEN 15 ELSE 30 END || ' minutes')<datetime(?) AND datetime(flight_start_at,'+'||(duration + CASE WHEN duration=30 THEN 15 ELSE 30 END)||' minutes')>datetime(?)").bind(simulator,candidateWindow.endsAt,candidateWindow.startsAt).first()) return Response.json({ error: "Der Simulator ist einschließlich Vor- und Nachbereitung bereits belegt." }, { status: 409 });
  ...
  if (instructorId !== null) {
    ...
    if (!instructor || !bookingFitsAvailability(flightTime,duration,instructor.available_from,instructor.available_until)) return Response.json({ error: "Der Instructor ist für den Termin einschließlich Vor- und Nachbereitung nicht verfügbar." }, { status: 409 });
    if (await db.prepare("SELECT id FROM bookings WHERE instructor_id=? AND status!='cancelled' AND datetime(flight_start_at, '-' || CASE WHEN duration=30 THEN 15 ELSE 30 END || ' minutes')<datetime(?) AND datetime(flight_start_at,'+'||(duration + CASE WHEN duration=30 THEN 15 ELSE 30 END)||' minutes')>datetime(?)").bind(instructorId,candidateWindow.endsAt,candidateWindow.startsAt).first()) return Response.json({ error: "Der Instructor ist einschließlich Vor- und Nachbereitung bereits eingeplant." }, { status: 409 });
    ...
    if(dayAssignment&&bookingFitsAvailability(flightTime,duration,dayAssignment.available_from,dayAssignment.available_until)){const conflict=await db.prepare("SELECT id FROM bookings WHERE instructor_id=? AND status!='cancelled' AND datetime(flight_start_at, '-' || CASE WHEN duration=30 THEN 15 ELSE 30 END || ' minutes')<datetime(?) AND datetime(flight_start_at,'+'||(duration + CASE WHEN duration=30 THEN 15 ELSE 30 END)||' minutes')>datetime(?)").bind(dayAssignment.instructor_id,candidateWindow.endsAt,candidateWindow.startsAt).first();if(!conflict){instructorId=dayAssignment.instructor_id;assignmentSource="day";}}
  }
