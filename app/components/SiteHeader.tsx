...
export function SiteHeader({ dark = false }: { dark?: boolean }) {
  ...
  return (
    <header className={"site-header " + (dark ? "solid-header" : "")}>
      <Link className="brand" href={english ? "/en" : "/"} aria-label={english ? "Vienna Flight home" : "Vienna Flight Startseite"}><span className="brand-mark">VF</span><span>VIENNA <b>FLIGHT</b></span></Link>
      <nav className={open ? "nav open" : "nav"} aria-label={english ? "Main navigation" : "Hauptnavigation"}>
        {items.map(([href, label]) => <Link href={href} key={href} aria-current={pathname === href ? "page" : undefined} onClick={() => setOpen(false)}>{label}</Link>)}
      </nav>
      <Link className="language-switch" href={languageHref} aria-label={english ? "Zur deutschen Version" : "Switch to English"}>{english ? "DE" : "EN"}</Link>
      <Link className="header-cta" href={english ? "/en/booking" : "/buchen"} aria-current={["/buchen", "/en/booking"].includes(pathname)}>{english ? "Book a flight" : "Flug buchen"} <span>↗</span></Link>
